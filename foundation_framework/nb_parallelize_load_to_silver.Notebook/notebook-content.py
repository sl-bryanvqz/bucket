...
def deduplicate_tasks_with_pairs(task_list):
    ...
    For duplicate task_ids, create a list of dicts with (task_log_id, run_id) pairs.
    ...
    for task_id, items in grouped_by_task_id.items():
        if False: #len(items) == 1:
            ...
            # Remove individual task_log_id and run_id since we have pairs
...
notebook_name = 'nb_load_to_silver'
...
try:
    ...
    task_metadata = eval(silver_runs_to_execute.replace("null",'None'))
