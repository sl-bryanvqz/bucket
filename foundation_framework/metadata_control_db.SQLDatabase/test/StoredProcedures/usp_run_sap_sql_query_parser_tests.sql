CREATE PROCEDURE [test].[usp_run_sap_sql_query_parser_tests]
AS
BEGIN
    /*
    Runs a test suite for: meta.usp_sap_sql_query_parser
    ...
    Assumptions:
    - meta.usp_sap_sql_query_parser returns a single-row, single-column result set
    ...
    */
    ...
    -- Test 1b - Updated TODAY_SAP1() with a value
    INSERT INTO #Tests (test_name, input_json, expect_error, assert_type, assert_value, notes)
    VALUES
    (
     N'T1b TODAY_SAP1 expanded (no literal remains)',
     N'{"source_query":"SELECT * FROM $main_table WHERE $varb1 >= ''$start_date''",
        "values":[{"main_table":"SAPCRM.CRMD_ISUEXTA4"},
                  {"varb1":"A4CONTSTART"},
                  {"start_date":"TODAY_SAP1()"}]}',
    ...
    -- Test 2 — NOW() returns Mountain time (ISO with offset -06:00 or -07:00)
    INSERT INTO #Tests (test_name, input_json, expect_error, assert_type, assert_value, notes)
    VALUES
    (
     N'T2 NOW() Mountain time ISO',
    ...
    -- Test 3 — NOW_SAP2() => yyyymmddHHMMSS (14 digits)
    INSERT INTO #Tests (test_name, input_json, expect_error, assert_type, assert_value, notes)
    VALUES
    (
     N'T3 NOW_SAP2 14-digit timestamp',
    ...
    -- Test 4 — ${token} syntax support (if your proc supports it)
    INSERT INTO #Tests (test_name, input_json, expect_error, assert_type, assert_value, notes)
    VALUES
    (
     N'T4 ${token} syntax',
    ...
    -- Test 5 — Overlapping tokens (id vs id2) / longest-first ordering
    INSERT INTO #Tests (test_name, input_json, expect_error, assert_type, assert_value, notes)
    VALUES
    (
     N'T5 Overlapping tokens id/id2',
    ...
    -- Test 6 — Token appears multiple times (3 occurrences)
    INSERT INTO #Tests (test_name, input_json, expect_error, assert_type, assert_value, notes)
    VALUES
    (
     N'T6 Token appears multiple times',
    ...
    -- Test 7 — Missing token mapping (unresolved allowed -> token remains)
    INSERT INTO #Tests (test_name, input_json, expect_error, assert_type, assert_value, notes)
    VALUES
    (
     N'T7 Missing token mapping remains unresolved',
    ...
    -- Test 8 — Empty string token value
    INSERT INTO #Tests (test_name, input_json, expect_error, assert_type, assert_value, notes)
    VALUES
    (
     N'T8 Empty string token value',
    ...
    -- Test 9 — Invalid JSON should throw
    INSERT INTO #Tests (test_name, input_json, expect_error, assert_type, assert_value, notes)
    VALUES
    (
     N'T9 Invalid JSON throws',
    ...
    -- Test 10 — Missing source_query should throw
    INSERT INTO #Tests (test_name, input_json, expect_error, assert_type, assert_value, notes)
    VALUES
    (
     N'T10 Missing source_query throws',
     N'{"values":[{"a":"b"}]}',
     1,
     NULL,
     NULL,
    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            EXEC meta.usp_sap_sql_query_parser @input_json;
